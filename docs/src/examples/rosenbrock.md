...
sol = solve(prob, Optim.SimulatedAnnealing())
...
sol = solve(prob, Optim.IPNewton()) # Note that -Inf < x[1]^2 + x[2]^2 < Inf is always true

prob = SciMLBase.OptimizationProblem(optf, x0, _p, lcons = [-5.0], ucons = [10.0])
sol = solve(prob, Optim.IPNewton()) # Again, -5.0 < x[1]^2 + x[2]^2 < 10.0
...
sol = solve(prob, Optim.IPNewton()) # -Inf < cons_circ(sol.u, _p) = 0.25^2
```
...
sol = solve(prob, NLopt.Opt(:LD_LBFGS, 2))
sol = solve(prob, NLopt.Opt(:G_MLSL_LDS, 2), local_method = NLopt.Opt(:LD_LBFGS, 2), maxiters = 10000) #a global optimizer with random starts of local optimization
```
