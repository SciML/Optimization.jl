...
function separate_kwargs(orig_kwargs)
    sub_options = :sub_options .∈ Ref(keys(orig_kwargs)) ? orig_kwargs[:sub_options] : nothing
    convergence_criteria = :convcriteria .∈ Ref(keys(orig_kwargs)) ? orig_kwargs[:convcriteria] : nothing
    opt_kwargs_names = keys(orig_kwargs)[[(keys(orig_kwargs) .∉ Ref([:sub_options, :convcriteria]))...]]
    opt_kwargs = (; zip(opt_kwargs_names, [getproperty(orig_kwargs, j) for j in opt_kwargs_names])...)
...
function __solve(prob::OptimizationProblem, opt::Nonconvex.NonconvexCore.AbstractOptimizer;
                 ...
                 reltol::Union{Number, Nothing}=nothing,
                 progress = false,
                 surrogate_flag::Union{Symbol, Nothing}=nothing,
                 integer=nothing,
                 kwargs...)
    ...
    if isnothing(surrogate_flag)
        Nonconvex.set_objective!(opt_set, opt_f)
    else
        Nonconvex.set_objective!(opt_set, opt_f, flag=surrogate_flag)
    end
    ...
    opt_ret = hasproperty(opt_res,:status) ? opt_res.status : nothing
