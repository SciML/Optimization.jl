...
function instantiate_function(f, cache::ReInitCache,
    ...
    pairs_arr = cache.p isa SciMLBase.NullParameters ?
                [Symbol(_s) => Expr(:ref, :x, i) for (i, _s) in enumerate(sys.states)] :
                [
        [Symbol(_s) => Expr(:ref, :x, i) for (i, _s) in enumerate(sys.states)]...,
        [Symbol(_p) => Expr(:ref, :p, i) for (i, _p) in enumerate(sys.ps)]...,
    ]
    ...
    if f.cons === nothing
        ...
        cons_sys = ModelingToolkit.modelingtoolkitize(NonlinearProblem(cons_oop, x,
                                                                       cache.p))
