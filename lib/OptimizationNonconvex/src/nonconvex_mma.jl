...
function convert_common_kwargs(
    opt::Union{NonconvexMMA.MMA02,NonconvexMMA.MMA87},
    opt_kwargs;
    callback = nothing,
    maxiters = nothing,
    maxtime = nothing,
    ...
    if !isnothing(abstol)
        tol_tmp =
            :tol .∈ Ref(keys(conv_opt_kwargs)) ? conv_opt_kwargs[:tol] :
            Nonconvex.NonconvexCore.Tolerance()
        tol_tmp_args = (;
            zip(
                propertynames(tol_tmp),
        ...
        conv_opt_kwargs =
            (; conv_opt_kwargs..., tol = Nonconvex.NonconvexCore.Tolerance(tol_tmp_args...))
    end
...
function __create_options(
    ...
    options =
        !isnothing(opt_kwargs) ? NonconvexMMA.MMAOptions(; opt_kwargs...) :
...
function _create_options(
    ...
    if !isnothing(convergence_criteria)
        options = (;
            options = __create_options(opt, opt_kwargs = opt_kwargs),
