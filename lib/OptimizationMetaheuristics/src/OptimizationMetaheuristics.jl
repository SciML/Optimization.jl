...
function initial_population!(opt, prob, bounds, f)
    ...
    population_rand = [
        bounds[1, :] + rand(length(prob.u0)) .* (bounds[2, :] - bounds[1, :]) for
        i = 1:(pop_size-1)
    ]
    push!(population_rand, prob.u0)
    population_init = [Metaheuristics.create_child(x, f(x)) for x in population_rand]
    prev_status =
        Metaheuristics.State(Metaheuristics.get_best(population_init), population_init)
...
function __map_optimizer_args!(
    ...
    for j in kwargs
        if j.first .∈ Ref(propertynames(Metaheuristics.Information()))
            error(
                "Set $(j.first) by directly passing it to Information Structure which is passed to $(typeof(opt)) algorithms when calling solve().",
            )
        elseif j.first .∈ Ref(propertynames(Metaheuristics.Options()))
            setproperty!(opt.options, j.first, j.second)
        else
            error(
                "$(j.first) keyword is not a valid option for $(typeof(opt).super) algorithm.",
            )
        end
    end
...
function SciMLBase.__solve(
    ...
    SciMLBase.build_solution(
        ...
        Metaheuristics.minimum(opt_res);
        original = opt_res,
    )
end
