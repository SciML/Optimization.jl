...
@testset "OptimizationSciPy.jl" begin
    ...
    @testset "ScipyMinimizeScalar" begin
        ...
        @test sol.retcode == ReturnCode.Success
        @test length(sol.u) == 1
        @test abs(2 * (sol.u[1] - p_scalar[1]) + cos(sol.u[1])) < 1.0e-6
    ...
    @testset "ScipyRootScalar" begin
        ...
        @test sol.retcode == ReturnCode.Success
        @test abs(f_root(sol.u, nothing)) < 1.0e-10
    ...
    @testset "constrained optimization" begin
        ...
        prob_cobyla = OptimizationProblem(optprob, x0, _p, lcons = [-1.0e-6], ucons = [1.0e-6])
    ...
    @testset "method-specific options" begin
        ...
        sol = solve(
            unconstrained_prob, ScipyDifferentialEvolution(),
        ...
        @test sol.retcode == ReturnCode.Success
        sol = solve(
            unconstrained_prob, ScipyDualAnnealing(),
            initial_temp = 5000.0, restart_temp_ratio = 2.0e-5
        )
        @test sol.retcode == ReturnCode.Success
    ...
    @testset "tolerance settings" begin
        optprob = OptimizationFunction(rosenbrock, OptimizationBase.AutoZygote())
        prob = OptimizationProblem(optprob, x0, _p)
        sol = solve(prob, ScipyNelderMead(), abstol = 1.0e-8)
        @test sol.objective < 1.0e-7
        sol = solve(prob, ScipyBFGS(), reltol = 1.0e-8)
        @test sol.objective < 1.0e-7
    end
    ...
    @testset "ScipyLinprog matrix constraints" begin
        ...
        b_ub = [5.0]
        sol = solve(
            prob_lp, ScipyLinprog("highs"),
            A_ub = A_ub, b_ub = b_ub,
            lb = [0.0, 0.0], ub = [10.0, 10.0]
        )
        @test sol.retcode == ReturnCode.Success
        @test sol.u[1] + sol.u[2] ≤ 5.0 + 1.0e-8
    end
    ...
    @testset "ScipyMilp matrix constraints" begin
        ...
        if sol.retcode == ReturnCode.Success
            ...
            @test isapprox(sol.u[1] + sol.u[2], 1.0; atol = 1.0e-6) ||
                sol.u[1] + sol.u[2] > 1.0
        end
    end
end
