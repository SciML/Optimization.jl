...
function _test_sparse_derivatives_hs071(backend, optimizer)
    ...
    @test isapprox(sol.objective, 17.014017145179164; rtol = 1.0e-1)
    x = [1.0, 4.742999641809297, 3.8211499817883077, 1.3794082897556983]
    @test isapprox(sol.u, x; rtol = 1.0e-1)
    @test prod(sol.u) >= 25.0 - 1.0e-6
    @test isapprox(sum(sol.u .^ 2), 40.0; rtol = 1.0e-1)
    return
end
...
@testset "Integer Support" begin
    ...
    @testset "Integer Domain" begin
        ...
        optfun = OptimizationFunction(
            (u, p) -> sum(abs2, x * u[1] .- y),
            AutoForwardDiff()
        )

        optprob = OptimizationProblem(
            optfun, u0; lb = one.(u0), ub = 6.0 .* u0,
            int = ones(Bool, length(u0))
        )
...
@testset "cache" begin
    ...
    sol = solve!(cache)
    @test sol.u ≈ [1.0] rtol = 1.0e-3 # ≈ [1]

    @test_broken begin
        cache = OptimizationMOI.reinit!(cache; p = [2.0])
        sol = solve!(cache)
        @test sol.u ≈ [2.0] rtol = 1.0e-3 # ≈ [2]
    end
end
