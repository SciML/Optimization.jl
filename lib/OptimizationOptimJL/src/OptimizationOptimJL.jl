...
function ___solve(
    ...
    function _cb(trace)
        cb_call =
            opt isa Optim.NelderMead ?
            callback(decompose_trace(trace).metadata["centroid"], x...) :
            callback(decompose_trace(trace).metadata["x"], x...)
    ...
    !(opt isa Optim.ZerothOrderOptimizer) &&
        f.grad === nothing &&
        error(
            "Use OptimizationFunction to pass the derivatives or automatically generate them with one of the autodiff backends",
        )
    ...
    if opt isa Optim.KrylovTrustRegion
        ...
        optim_f = Optim.TwiceDifferentiable(
            ...
            prob.u0,
            real(zero(F)),
            Optim.NLSolversBase.alloc_DF(prob.u0, real(zero(F))),
            isnothing(f.hess_prototype) ?
            Optim.NLSolversBase.alloc_H(prob.u0, real(zero(F))) :
    ...
    SciMLBase.build_solution(
        ...
        opt,
        opt_res.minimizer,
        prob.sense === Optimization.MaxSense ? -opt_res.minimum : opt_res.minimum;
        original = opt_res,
        retcode = opt_ret,
    )
end
...
function ___solve(
    prob::OptimizationProblem,
    opt::Union{Optim.Fminbox,Optim.SAMIN},
    data = Optimization.DEFAULT_DATA;
    callback = (args...) -> (false),
    maxiters::Union{Number,Nothing} = nothing,
    maxtime::Union{Number,Nothing} = nothing,
    ...
    function _cb(trace)
        cb_call =
            !(opt isa Optim.SAMIN) && opt.method == Optim.NelderMead() ?
            callback(decompose_trace(trace).metadata["centroid"], x...) :
            callback(decompose_trace(trace).metadata["x"], x...)
        if !(typeof(cb_call) <: Bool)
            error(
...
function ___solve(
    prob::OptimizationProblem,
    opt::Optim.ConstrainedOptimizer,
    data = Optimization.DEFAULT_DATA;
    callback = (args...) -> (false),
    maxiters::Union{Number,Nothing} = nothing,
    maxtime::Union{Number,Nothing} = nothing,
    ...
    cons_hl! = function (h, θ, λ)
        res = [similar(h) for i = 1:length(λ)]
        f.cons_h(res, θ)
        for i = 1:length(λ)
            h .+= λ[i] * res[i]
        end
    ...
    optim_fc = Optim.TwiceDifferentiableConstraints(
        cons!,
        cons_j!,
        cons_hl!,
        lb,
