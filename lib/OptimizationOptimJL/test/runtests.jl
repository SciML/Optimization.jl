using OptimizationOptimJL, OptimizationOptimJL.Optim, Optimization, ForwardDiff, Zygote, Random, ModelingToolkit
using Test
...
@testset "OptimizationOptimJL.jl" begin
    ...
    optprob = OptimizationFunction((x, p) -> -rosenbrock(x, p), Optimization.AutoZygote(), grad=g!)
    prob = OptimizationProblem(optprob, x0, _p; sense=Optimization.MaxSense)
    sol = solve(prob, BFGS())
    @test 10 * sol.minimum < l1
