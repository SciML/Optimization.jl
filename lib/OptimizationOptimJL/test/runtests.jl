using OptimizationOptimJL,
    OptimizationOptimJL.Optim, Optimization, ForwardDiff, Zygote, Random, ModelingToolkit
using Test
...
@testset "OptimizationOptimJL.jl" begin
    ...
    sol = solve(
        prob,
        Optim.NelderMead(; initial_simplex = Optim.AffineSimplexer(; a = 0.025, b = 0.5)),
    )
    @test 10 * sol.minimum < l1
    ...
    optprob =
        OptimizationFunction(rosenbrock, Optimization.AutoModelingToolkit(); cons = cons)
    ...
    optprob =
        OptimizationFunction(rosenbrock, Optimization.AutoForwardDiff(); cons = con2_c)
    ...
    optprob =
        OptimizationFunction(rosenbrock, Optimization.AutoForwardDiff(); cons = cons_circ)
    ...
    optprob =
        OptimizationFunction(rosenbrock, Optimization.AutoModelingToolkit(true, false))
    prob = OptimizationProblem(optprob, x0, _p)
    sol = solve(prob, Optim.Newton())
    @test 10 * sol.minimum < l1
