...
function SciMLBase.__solve(
    ...
    if splits === nothing
        error(
            "You must provide the initial locations at which to evaluate the function in `splits` (a list of 3-vectors with values in strictly increasing order and within the specified bounds).",
        )
    end
    ...
    SciMLBase.build_solution(
        prob,
        opt,
        QuadDIRECT.position(box, x0),
        QuadDIRECT.value(box);
        original = root,
    )
end
